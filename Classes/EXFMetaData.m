...
- (instancetype) init
{
    return [self initWith:0 :1];
}

-(instancetype) initWith: (long) aNumerator : (long) aDenominator{
...
-(NSString*) description{
    ...
    return [NSString stringWithFormat:@"%@",@((double)numerator/(double) denominator)];
}
@end
...
- (instancetype) init
{
    return [self initWith:0 :0 :nil :0 :false :0];  // NEVER CALL THIS
}

-(instancetype) initWith: (EXFTagId) aTag : (EXFDataType)aDataType : (NSString*) aName : (int) aParentTagId : (BOOL)isEditable : (int)theComponets{
    if (self = [super init]) {
        self.tagId = aTag;
...
-(EXFTag*) tagDefinition: (NSNumber*) aTagId{
    return (self.tagDefinitions.definitions)[aTagId];
}
...
-(NSMutableArray*) tagDefinitionsForParent:(NSNumber*) parent withoutImmutable:(BOOL) includeImmutable {
                                ...
                                for(id key in keys){
                                                                EXFTag* tag = (self.tagDefinitions.definitions)[key];
...
-(id) tagValue: (NSNumber*) aTagId{
    ...
    id value = (self.keyedTagValues)[aTagId];
    
    if (value == nil){
    
       EXFTag* tag = (self.tagDefinitions.definitions)[aTagId];
       
       if (tag != nil){
           NSDictionary* dict = (self.keyedTagValues)[@(tag.parentTagId)];
           if (dict != nil){
               value = dict[aTagId];
           }
           
       }else{
            NSDictionary* dict = (self.keyedTagValues)[@EXIF_GPS];
            value = dict[aTagId];
            
            if (value ==nil){
                dict = (self.keyedTagValues)[@EXIF_Exif];
                value = dict[aTagId];
            }
       }
...
-(id) thumbnailTagValue: (NSNumber*) aTagId{
    return (self.keyedThumbnailTagValues)[aTagId];
}
...
-(void) addHandler:(id)handler : (UInt16) keyValue{
    ...
    (self.keyedHandlers)[_key] = handler;
...
-(void) addHandler:(id<EXFTagHandler>) aTagHandler forKey:(NSNumber*) aKey{
    ...
    // now check optional conformance - if no tag exists then it must implement all the methods
     EXFTag* tag =  (self.keyedTagDefinitions)[aKey];
     if (tag == nil){
    ...
    (self.userKeyedHandlers)[aKey] = aTagHandler;
}
...
-(void) addTagValue:(id)value forKey:(NSNumber*) aTagKey {
    ...
    EXFTag* tag =  (self.keyedTagDefinitions)[aTagKey];
    
    int parentTagId = -1;
    
    // see if we have a user registered handler for support
    id<EXFTagHandler> handler = (self.userKeyedHandlers)[aTagKey];
    
    if (handler == nil){
        //see if we have one of our default handlers
        handler = (self.keyedHandlers)[aTagKey];
    }
                ...
                /*
                    if (! [tempValue isKindOfClass:[NSNumber class]]  ||
                        [dataTypeStr rangeOfString:[NSString stringWithFormat:@"%s",[tempValue objCType]]].location == NSNotFound) 
                       {
                        NSException *e = [NSException
                                          exceptionWithName:@"InvalidTypeException"
                                          reason:[NSString stringWithFormat: @"Tag %@ does not support numeric type %s for %@",aTagKey, [tempValue objCType],value]
                                          userInfo:nil];
                        @throw e;
                    }
                 */
    ...
    if (parentTagId != -1)
    {
        ...
        [parentTagNumber release];
                                ...
                                if ([dictionary count] ==0 && parentTagId != -1){
                                                                [self removeTagValue:@(parentTagId)];
                                }
...
-(void) assignElements: (NSMutableDictionary*) keyedValues : (NSNumber*) tag : (id) elements : (UInt32) components{
    if (components > 1){
        keyedValues[tag] = elements;
    }else{
        keyedValues[tag] = elements[0]; 
    }
}
...
-(int) processExifDir:(NSMutableDictionary*) keyedValues : (int) dirStart : (int) offsetBase : (BOOL) thumbnail{
    ...
    for (int de =0;de <numEntries;de++){
        ...
        NSInteger processedValueCount = [keyedValues count];
        ...
        if (byteCount > 4) {
            ...
            Debug(@"Offset %li found for tag %lu with bytecount %u",(long)valueOffset, (unsigned long) tag,(unsigned int)byteCount);
       ...
       // sometimes thumbnail has no tag just runs on end of data - so keep track of where we are in block
        ...
        if (tag == TAG_EXIF_OFFSET || tag == TAG_INTEROP_OFFSET || tag == TAG_EXIF_GPS) {
            ...
            Debug(@"Nested pointer to %lu found for tag %i with bytecount %u",(long) subdirOffset +offsetBase, tag,(unsigned int)byteCount);
            ...
            id<EXFTagHandler> handler = [self userKeyedHandlers][tagNumber];
            
            // if not see if we have a default specific handler
            if (handler == nil){
                handler = [self keyedHandlers][tagNumber];

            }
            
            // a handler is dealing with this tag
            if (handler != nil){
                Debug(@"Handler %@ invoked for tag %@ invoked with offset %u and byte count %u",handler, tagNumber, (unsigned int)valueOffset, (unsigned int)byteCount);
                ...
                EXFTag* tagDefinition = (self.keyedTagDefinitions)[tagNumber];
    ...
    Debug(@"Next Offset %u at %i",(unsigned int)nextOffset, nextIFD);
    ...
    if(thumbnail){
        ...
        NSNumber* jpegOffset = keyedValues[@EXIF_JPEGInterchangeFormat];
        
        // if we have a tag identifying offset we should be able to get length
        if (jpegOffset != nil){
            // get the length
            NSNumber* jpegLength = keyedValues[@EXIF_JPEGInterchangeFormatLength];
        ...
        if (thumbnailLength >0 && thumbnailStart + thumbnailLength == self.byteLength){
            // looks ok - try and get remaining block as thumbnail
                ...
                Debug(@"*** Thumbnail start of %li and length %li is more than total block length %li", thumbnailStart, thumbnailLength, self.byteLength);
        }else{
                
                Debug(@"*** Thumbnail start of %li and length %li is less than total block length %li", thumbnailStart, thumbnailLength, self.byteLength);
        }
    }
        // if there has not been an offset value 
       
    
      Debug(@"********** Leaving exif processing at %i with nextOffset %u***********",dirStart  , (unsigned int)nextOffset);
    return nextOffset;
}
...
-(void) parseExif: (CFDataRef*) exifData
 {
    ...
    // get the byte length
     ...
     Debug(@"Length of exif %li", byteLength);
     
     // must be at least 13 bytes
     if (self.byteLength <13){
         NSLog(@"***Warning byte length for EXIF is too short %li must be at least 13 bytes", self.byteLength);
         return;
     }
    ...
    // get the endian of the bytes
     UInt8 order[2];
     CFDataGetBytes(*exifData, CFRangeMake(6,2), order); // see http://code.google.com/p/iphone-exif/issues/detail?id=4&can=1
...
-(void) writeDataToBuffer: (NSMutableData*) target : (id) obj : (int) dataType : (int) tagByteSize : (UInt8**) bytes{
    ...
    switch (dataType) {
        case FMT_UNDEFINED:{
            ...
            break;
        case FMT_STRING:{
            Debug(@"Writing ASCII String %@ with count of %i", obj, tagByteSize);
...
-(void) getDataFromMap: (NSDictionary*) dictionary :(NSMutableArray*) dataWriterArray : (UInt8**) bytes : (int) overflowOffset : (int) offsetBase{ 
    ...
    NSInteger size = [sortedKeysArray count] ;
    
    
    // the extra plus 4 is where the next ifd for any thumbnail is set
    // if no value then 00000000 pads between blocks 
    NSInteger blockCount = (size *12 +2)  +4;
    
    Debug(@"Number of ELements %li and block size of %li",(long)size, (long)blockCount);
   
    // write the number of elements in the first 2 bytes
    [EXFUtils write2Bytes:bytes :@((int)size) :self.bigEndianOrder];
    ...
    for (int de =0;de <size;de++) {
        ...
        NSNumber *key = sortedKeysArray[de];
        ...
        if ([obj isKindOfClass:[NSDictionary class]]){
            ...
            EXFTag* tag = (self.keyedTagDefinitions)[key];
            ...
            id<EXFTagHandler> handler = [self userKeyedHandlers][key];
                
            // if not see if we have a default handler
            if (handler == nil){
                handler = [self keyedHandlers][key];    
            }
            ...
            if (handler != nil){
                ...
                if (tag.components <0){
                    ...
                    if ([obj isKindOfClass:[NSData class]]){
                        ...
                        tagComponentSize = (int) [((NSData*)obj) length];
                    } else  if ([obj isKindOfClass:[NSString class]]){
                        // String length is determined by ascii encoding only
                        tagComponentSize = (int) [((NSString*)obj) lengthOfBytesUsingEncoding:NSASCIIStringEncoding];
            ...
            if (tagByteSize <0){
                ...
                Debug(@"Writing value for key %@ at final location %lu",key, (long) [dataWriter.tagData length] + overflowOffset + offsetBase);
                ...
                // write the data type
                [EXFUtils write2Bytes:bytes :@(tagDataType) :self.bigEndianOrder];
                [dataWriter.tagData appendBytes:*bytes length:2];
                
                // now write the count
                Debug(@"Writing byte count of %i key %@ ",tagComponentSize, key);
                [EXFUtils write4Bytes:bytes :@(tagComponentSize) :self.bigEndianOrder];
                ...
                // now set up the data elements to be written
                 if (tagByteSize <=4){
                     ...
                     NSInteger temp = blockCount + overflowOffset + [dataWriter.overflowData length];
                     
                     Debug(@"Writing overflow location of %li (%li) for key %@ ",(long)temp, (long) temp+offsetBase, key);
                     
                     [EXFUtils write4Bytes:bytes :@((int) temp) :self.bigEndianOrder];
                     [dataWriter.tagData appendBytes:*bytes length:4]; 
                    
                      Debug(@"Length of tagData %lu ",(unsigned long)[dataWriter.tagData length]);
                 }
                ...
                if (handler != nil){
                    ...
                    if ([tagData length] != tagByteSize){
                        NSLog(@"***Warning Handler %@ returned %i bytes for tag %@ - expected %i. Altering buffer size to projected size", handler, (int) [tagData length],key,tagByteSize);
                        // we should truncate or expand based on the value here
                      [tagData setLength:tagByteSize];
                    } 
                    // pad if we need to
                    if ([tagData length] <4){
                        Debug(@"Tag data less than 4 for tag %@ from handler %@ - padding %i bytes", key, handler, (int) (4 - [tagData length]));
                ...
                Debug(@"Overflow size of %i", (int) [dataWriter.overflowData length]);
    ...
    int nestedOffset= (int) (overflowOffset + blockCount + [dataWriter.overflowData length] );
    
    for(int i=0;i< [nestedTags count];i++){
        
        NSNumber* key = (NSNumber*) nestedTags[i];
        
        
        Debug(@"Writing value for key %@ at location %ld",key, (long) ([dataWriter.tagData length] + overflowOffset + offsetBase));
        ...
        // write the data type
        [EXFUtils write2Bytes:bytes :@(FMT_ULONG) :self.bigEndianOrder];
        [dataWriter.tagData appendBytes:*bytes length:2];
        
        // now write the count
        [EXFUtils write4Bytes:bytes :@1 :self.bigEndianOrder];
        [dataWriter.tagData appendBytes:*bytes length:4];
        ...
        [EXFUtils write4Bytes:bytes :@(nestedOffset) :self.bigEndianOrder];
        [dataWriter.tagData appendBytes:*bytes length:4];
        
        // now process the nested tag
        [self getDataFromMap:dictionary[key]: dataWriterArray: bytes: nestedOffset: offsetBase];
...
-(void) getData: (NSMutableData*)imageData {
    ...
    int initialSize = (int) [imageData length];
    ...
    for(int i=0;i<[dataWriters count];i++){
        ...
        if (i ==0 && [self.keyedThumbnailTagValues count] >0){
            ...
            thumbnailOffsetPointer = (int) [imageData length];
    ...
    // for each dictionary get the 
     ...
     if (thumbnailOffsetPointer != 0){
         ...
         [EXFUtils write4Bytes:&ptr :@((int) ([imageData length] -initialSize) -6):TRUE];
         Debug(@"Replacing bytes at %i with thumbnailOffset Pointer %i",thumbnailOffsetPointer,(int)([imageData length] -initialSize)-6);
         ...
         if (self.thumbnailBytes != nil){
             Debug(@"Adding %i thumbnail bytes to block",(int) [self.thumbnailBytes length]);
